...
(function () {
  ...
  // Number Match: exactly one number appears twice, in two different suits.
  // Every other numbered tile has its own number, and the rest of the board is
  // Winds, Dragons, Flowers and Jokers, which have no number. One right answer.
  var currentNum = null;
  function pickOf(arr) { return arr[Math.floor(Math.random() * arr.length)]; }
  function newList() {
    var nums = shuffle([1, 2, 3, 4, 5, 6, 7, 8, 9]);
    currentNum = nums[0];
    var suits = shuffle(['Crak', 'Dot', 'Bam']);
    var find = function (s, n) { return DESIGNS.filter(function (d) { return d.suit === s && d.num === n; })[0]; };
    var list = [find(suits[0], currentNum), find(suits[1], currentNum)];
    var others = size - 2, numbered = Math.min(8, others - 2);
    nums.slice(1, 1 + numbered).forEach(function (n) { list.push(find(pickOf(['Crak', 'Dot', 'Bam']), n)); });
    var honors = shuffle(DESIGNS.filter(function (d) { return !d.num; }));
    list = list.concat(honors.slice(0, size - list.length));
    return shuffle(list);
  }
  function isPair(a, b) { return a.num && a.num === b.num && a.suit !== b.suit; }
  function plural(s) { return s + 's'; }
  function guide(a, b) {
    var noNum = !a.num ? a : (!b.num ? b : null);
    if (noNum) return plural(noNum.kind.replace(/s$/, '')) + ' have no number';
    if (a.num !== b.num) return a.num + ' and ' + b.num + ' are different numbers';
    return '';
  }
  function showCap(t) {
  ...
  function onTile(e) {
    ...
    if (isPair(tiles[picked], tiles[i])) {
      var pairMsg = tiles[picked].num + ' ' + tiles[picked].suit + ' and ' + tiles[i].num + ' ' + tiles[i].suit + ': same number, different suits';
      locked = true;
  ...
  function hint() {
    if (locked) return;
    var pair = [];
    tiles.forEach(function (d, i) { if (d.num === currentNum) pair.push(i); });
  ...
  function setSize(n) {
    try { localStorage.setItem('mk-number-match-size', String(n)); } catch (e) { /* storage off: fine */ }
  ...
  var savedSize = null;
  try { savedSize = Number(localStorage.getItem('mk-number-match-size')); } catch (e) { /* storage off */ }
  setSize([9, 16].indexOf(savedSize) !== -1 ? savedSize : 16);
})();
