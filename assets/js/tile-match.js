...
(function () {
  ...
  function onTile(e) {
    ...
    if (tiles[picked] === tiles[i]) {
      ...
      if (cap) { cap.textContent = tiles[i].name; cap.classList.remove('is-shown'); void cap.offsetWidth; cap.classList.add('is-shown'); }
  ...
  function setSize(n) {
    try { localStorage.setItem('mk-tile-match-size', String(n)); } catch (e) { /* storage off: fine */ }
  ...
  var savedSize = null;
  try { savedSize = Number(localStorage.getItem('mk-tile-match-size')); } catch (e) { /* storage off */ }
